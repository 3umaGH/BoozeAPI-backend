...
const MAX_RANDOM_COCKTAILS_PER_REQUEST = process.env.MAX_RANDOM_COCKTAILS_PER_REQUEST || 50;

router.get(
  "/random",
  [
    query("amount").trim().escape(),
  ],
  async (req, res) => {
    try {
      ...
      if(amount > MAX_RANDOM_COCKTAILS_PER_REQUEST)
        return res.status(400).json({ message: `Maximum ${MAX_RANDOM_COCKTAILS_PER_REQUEST} cocktails per request.` });
